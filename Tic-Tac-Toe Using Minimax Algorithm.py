...
def changeModeToSinglePlayer(): 
    global mode #global variable mode is set
    mode = "singlePlayer" #set value of mode
    singlePlayerButton["bg"] = "lightgreen" #changing colors of buttons
    multiPlayerButton["bg"] = "lightgrey"

def changeModeToMultiplayer():
    global mode 
    mode = "multiPlayer" #similar to single mode
    multiPlayerButton["bg"] = "lightgreen"
    singlePlayerButton["bg"] = "lightgrey"


def updateBoard(): # updating the board each time for x and o
...
def restartGame(): #we have to empty each value
    global game_end #global end for restart
    game_end = False
    for button in buttons: #for button's text
        button["text"] = " "

    for i in board.keys():#for button's value
        board[i] = " "

    titleLabel = Label(frame1 , text="Tic Tac Toe" , font=("Arial" , 30) , bg="orange" , width=15 )
    titleLabel.grid(row=0 , column=0)

def minimax(board , isMaximizing): #defination of algorithm minmax(how will the computer will process the steps to win)
    ...
    if isMaximizing:
        ...
        for key in board.keys():
            if board[key] == " ":
                board[key] = "o" # o is used for computer
                score = minimax(board , False) # minimax recursion is used to get if maximum value is attained or not
                board[key] = " "
                if score > bestScore : 
                    bestScore = score 
        ...
        bestScore = 100
        # x is being checked here
        for key in board.keys():
            if board[key] == " ":
                board[key] = "x"
                score = minimax(board , True) # minimax
                board[key] = " "
                if score < bestScore : # comparing score(tries to win in every step) whether x or o will win
                    bestScore = score 
        
        return bestScore

def playComputer(): #implementation of algorithm minimax
    bestScore = -100
    bestMove = 0

    for key in board.keys():#optimal(best) location so that I Can win then I will write 'o' on the board
        if board[key] == " ":
            board[key] = "o"
...
def play(event):
    ...
    button = event.widget #gui
    buttonText = str(button) #button's text is a string (x,o)
    clicked = buttonText[-1]
    if clicked == "n" : #default condition
        clicked = 1
    else :
        clicked = int(clicked)
    ...
    if button["text"] == " ":
        if turn == "x" :
            ...
            updateBoard()
            #if single is there then only this is executed
        ...
        else:# o's turn over then given to x
            board[clicked] = turn
            updateBoard()
            if checkForWin(turn):
                winningLabel = Label(frame1 , text=f"{turn} wins the game" , bg="orange", font=("Arial" , 26),width=16)
